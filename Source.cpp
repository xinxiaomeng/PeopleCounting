...
void Initial()//初始化
{
	...
	frameSize.height = (int) capture.get(CV_CAP_PROP_FRAME_HEIGHT);//获取帧的高度
	frameSize.width = (int) capture.get(CV_CAP_PROP_FRAME_WIDTH);//获取帧的宽度

	frameNum = (int) capture.get(CV_CAP_PROP_FRAME_COUNT);//获取总帧数
	frameIndex = 0;
}

void InitialImageMaxMin(const Mat* grayFrame)//初始化图像的最大最小值
{
	grayFrame->copyTo(iMatMax);//grayFrame是指向图像的指针
	grayFrame->copyTo(iMatMin);
}

void UpdateImageMaxMin(Mat* grayFrame)//更新图像的最大最小值
{
	int index = 0;
...
void UpdateBackgroundImage()//更新背景图像
{
	uchar diffImage;
	uchar meanImage;
